...
async def my_coins_handler(message: Message):
    ...
    await message.answer(f"💰 У вас {coins} STRAWBERRY Coin 🍓")
...
async def shop_set_description(message: Message, state: FSMContext):
    ...
    await message.answer("💰 Введите <b>стоимость товара</b> в STRAWBERRY Coin (только число):", parse_mode="HTML")
...
async def shop_set_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"💰 Стоимость: {data['price']} STRAWBERRY Coin",
        parse_mode="HTML"
    )
    await state.clear()
